 ...
 */
     ...
     : QAbstractListModel(parent)
 {
     ...
     d->model.setResolveSymlinks(true);
#endif
     ...
     connect(&d->watcher,SIGNAL(directoryChanged()),&d->model,SLOT(refresh()));
 }
 ...
 */
 ...
 void QDeclarativeFolderListModel::setFolder(const QUrl &folder)
 {
     ...
     if ((index.isValid() && d->model.isDir(index)) || folder.toLocalFile().isEmpty()) {
         QStringList watched = d->watcher.directories();
         d->watcher.removePaths(watched);
         d->watcher.addPath(folder.toString());
         d->folder = folder;
